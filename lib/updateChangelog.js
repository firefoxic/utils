...
function updateChangelogLinks (content, newVersion) {
	...
	const existingVersionRegex = new RegExp(`\\[${previousVersion}\\]:`)
	const newVersionLinkPath = content.match(existingVersionRegex)
		? `${linkMatch[1]}${previousVersion}...v${newVersion}`
		: `${linkMatch[1].replace(`compare`, `releases/tag`)}${newVersion}`

	const newLinks = `${linkMatch[1]}${newVersion}${linkMatch[3]}\n[${newVersion}]${newVersionLinkPath}`
